...
class AutoRegressiveSeqDecoder(SeqDecoder):
    ...
    def __init__(
        ...
        double_enc=False) -> None:
        super().__init__(target_embedder)
        # Set seeds again, better reproducibility
        fixed_seeds()
        self._vocab = vocab
        self._double_enc = double_enc

        self._multiplier = 2 if self._double_enc else 1
        ...
        # It's possible to add an extra linear layer (+ tanh) inbetween if wanted
        self._linear_projection = Linear(self._decoder_net.get_output_dim(), self._decoder_net.get_output_dim())
        self._output_projection = Linear(self._decoder_net.get_output_dim(), target_vocab_size)

        # Extra decoder layer
        self._output_projection_layer = torch.nn.Sequential(self._linear_projection, self._output_projection)

        # Set extra layer (tanh + linear) for after setting decoder context vector
        self._encoder_linear_layer = torch.nn.Sequential(Linear(self._decoder_net.get_output_dim(), target_vocab_size), torch.nn.Tanh())
        # Set extra layer for between encoder and decoder
        self._extra_enc_layer = torch.nn.Sequential(Linear(self._decoder_net.get_output_dim() * self._multiplier, self._decoder_net.get_output_dim()), torch.nn.Tanh())
    ...
    def     _forward_beam_search(self, state: Dict[str, torch.Tensor], target_tokens = None, source_tokens = None, vocab_cust = None)\
        ...
        all_top_k_predictions, log_probabilities, attention_alignment = self._beam_search.search(
            start_predictions, state, self.take_step, target_tokens, source_tokens, vocab_cust
        )
    ...
    def _forward_loss(
        self, state: Dict[str, torch.Tensor], target_tokens: Dict[str, torch.LongTensor],
            source_tokens = None, vocab_cust = None,
    ) -> Dict[str, torch.Tensor]:
        """
        ...
        if torch.is_tensor(target_tokens[self._target_namespace]):
            targets = target_tokens[self._target_namespace]
        else:
            targets = target_tokens["tokens"][self._target_namespace]
    ...
    def _prepare_output_projections(
        self, last_predictions: torch.Tensor, state: Dict[str, torch.Tensor],
            source_tokens = None, target_tokens = None, vocab_cust = None,
    ...
    def take_step(
        self, last_predictions: torch.Tensor, state: Dict[str, torch.Tensor],
            target_tokens = None, source_tokens = None, vocab_cust = None
    ) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        """
    ...
    def forward(
        ...
        vocab_cust = None, source_tokens = None,
    ) -> Dict[str, torch.Tensor]:
        state = encoder_out
        decoder_init_state = self._decoder_net.init_decoder_state(state, extra_enc_layer=self._extra_enc_layer)
        ...
        if not self.training:
            predictions = self._forward_beam_search(state, target_tokens, source_tokens, vocab_cust)
        ...
        if target_tokens:
                if self._tensor_based_metric is not None:
                    ...
                    self._tensor_based_metric(  # type: ignore
                        best_predictions, target_tokens["tokens"][self._target_namespace]
                    )
    ...
    def post_process_alignment(self, attends, idx) -> List[List[str]]:
        ...
        for (input_list, output_token_list, tensor) in attendsAggregated:
            ...
            alignment = ' % ' + most_mass_token + ' ' + str(char_count_previous_tokens) + ' ' + str(
                char_count_previous_tokens + len(most_mass_token))
