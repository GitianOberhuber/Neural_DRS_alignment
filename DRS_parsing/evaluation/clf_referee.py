...
def parse_arguments():
    ...
    parser.add_argument("-rt", "--reference_input_token", action='store_true',
                        help="Expect clauses to contain the natural language utterance it refers to as well as its\
                        start- and stop-indices as the last 3 elements of a clause (e.g hello 0 5) and handle accordingly")
    parser.add_argument(
...
#################################
def clf_typing(clf, signature, v=0, rt = False):
    ...
    for cl in clf:
        (op_type, typing) = clause_typing(cl, signature, v=v, rt = rt)
...
#################################
def clause_typing(clause, signature, v=0, rt = False):
    ...
    if rt:
        ...
        if len(clause) > 4:
            report_error("Too many elements in a clause || {}".format(clause), v=v)
    operator, arguments = clause[1],  (clause[0],) + clause[2:] #fx
...
#################################
def clf_to_box_dict(clf, op_types, arg_typing, v=0, rt = False):
