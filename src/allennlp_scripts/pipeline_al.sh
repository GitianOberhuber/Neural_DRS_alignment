...
pp_and_counter(){
    ...
    if [ -f "$2" ]; then
        if [[ ! -f "${2}.out" || $FORCE_PP = true ]]; then
            ...
            python $PP_PY --input_file $2 --output_file ${2}.out --sig_file $SIG_FILE --fix --json --sep $SEP -rcl $REMOVE_CLAUSES -m $MIN_TOKENS -al -voc $vocab_file $no_sep > ${1}log/$4_pp$3.log
