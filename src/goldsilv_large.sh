...
if $option_rt; then
	...
	while IFS= read -r line1 && IFS= read -r line2 <&3; do
		echo -e "${line1}\t${line2}"
	done < data/3.0.0/en/gold_silver/${type}.txt.raw.tok  3< data/3.0.0/en/gold_silver/${type}.txt.tgt.rt > data/3.0.0/en/gold_silver/${type}.rt.alp
    done
    ...
    CONFIG="config/allennlp/en_large/en_goldsilv_tok_fine/" #only difference from old config file: train_data_path points to gold data (as opposed to gold+silver)
else
    ...
    $PIPELINE ${CONFIG}/bert.json ${EXPS}/bert/ normal en
    echo "Fine-tuning model on gold data..." ;sleep 1
    CONFIG="config/allennlp/en_large/en_goldsilv_nontok_fine/" #only difference from old config file: train_data_path points to gold data (as opposed to gold+silver)
fi
