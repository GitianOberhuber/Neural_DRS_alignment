...
class RewriteVariables:
    ...
    def rewrite_variables(self):
        ...
        for cur_clause in self.drs:
            # Clause has 3 items
            if len(cur_clause) == 3:
                self.rewrite_length_three(cur_clause, False)
    ...
    def rewrite_variables_rt(self):
        ...
        if self.rtype == "none":
            return self.drs
        for cur_clause in self.drs:
            # Clause has 3 items (+3 items for the word it is reffering to and its start- and stop-index)
            if len(cur_clause) == 6:
                self.rewrite_length_three(cur_clause, True)
            # Clause has 4 items (+3), no introduction, only rewrites
            else:
                self.rewrite_length_four(cur_clause, True)
        return self.new_clauses
